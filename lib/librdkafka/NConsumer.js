...
class NConsumer extends EventEmitter {
  ...
  /**
   * fakes a message with an offset - 1 for the given 
   * topic and partition and makes an attempt to commit it
   * @param {string} topic 
   * @param {number} partition 
   * @param {number} offset 
   * @param {boolean} async 
   */
  commitOffsetHard(topic, partition, offset, async = false){

    const fakeMessage = {
      topic, 
      partition,
      offset: offset - 1 // noderd-kafka commitMessage => offset + 1
    };

    return this.commitMessage(async, fakeMessage);
  }

  /**
   * EXPERIMENTAL
   * pass a topic name and a partition to offset table (object, hashmap)
   * the consumer will fetch the latest metadata for the topic and iterate through
   * all the available partitions, attempting hard offset commits for the partitions
   * for which it can find mapping values in the provided parition mapping table
   * @param {string} topicName 
   * @param {{0: number}} offsetPartitionMappingTable - {{[partition.id]: offset}}
   * @returns {Array<number>} - the commited partitions
   */
  async commitOffsetForAllPartitionsOfTopic(topicName, offsetPartitionMappingTable){

    const metadata = await this.getTopicMetadata(topicName);
    const partitions = metadata.getPartitionsForTopic(topicName);

    const committedPartitions = [];
    partitions.forEach((partition) => {

      if(offsetPartitionMappingTable &&
        typeof offsetPartitionMappingTable[partition] !== "undefined"){
          this.commitOffsetHard(topicName, partition, offsetPartitionMappingTable[partition]);
          committedPartitions.push(partition);
        }
    });

    return particommittedPartitionstions;
  }

  /**
   * NOTICE: EXPERIMENTAL
   * attempts to commit to "offset beginning" for all partitions of a topic
   * @param {string} topicName 
   */
  async resetTopicPartitionsToEarliest(topicName){

    const metadata = await this.getTopicMetadata(topicName);
    const partitions = metadata.getPartitionsForTopic(topicName);

    partitions.forEach((partition) => {
      this.commitOffsetHard(topicName, partition, BlizzKafka.Topic.OFFSET_BEGINNING);
    });
  ...
  /**
   * NOTICE: EXPERIMENTAL
   * attempts to commit to "offset end" for all partitions of a topic
   * @param {string} topicName 
   */
  async resetTopicPartitionsToLatest(topicName){

    const metadata = await this.getTopicMetadata(topicName);
    const partitions = metadata.getPartitionsForTopic(topicName);

    partitions.forEach((partition) => {
      this.commitOffsetHard(topicName, partition, BlizzKafka.Topic.OFFSET_END);
    });
